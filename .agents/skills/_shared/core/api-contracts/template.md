...
> Author: {assigned-owner} | Session: {session-id} | Date: {YYYY-MM-DD}

Use only the operations and fields needed by the actual contract. The CRUD, UUID, ownership, and pagination examples below are placeholders, not required architecture.

## Base URL
`/api/{domain}`

## Authentication
{Actual authentication scheme, public operations, and per-operation authorization rules. Preserve the project contract; do not assume JWT.}
